...
-- We have changes incoming. Part 3 of the project gave some insights which we never looked into.

SELECT
    COUNT(*)
FROM aws_project.applicant_optimized;-- 1,25,69,167 (This count includes double means for an id we have 2 records with different name)

SELECT
    COUNT(*)
FROM aws_project.institute_optimized;-- 1,00,818

SELECT applicant_id_sk, COUNT(*) AS cnt
...
-- Removing one record out of the double.
BEGIN;

-- 1. Identify which IDs actually have duplicates (to save memory)
CREATE TEMP TABLE duplicate_ids AS
SELECT applicant_id_sk
FROM aws_project.applicant_optimized
GROUP BY
    applicant_id_sk
HAVING COUNT(*) > 1;

-- 2. Grab exactly ONE perfect copy of those duplicated records
CREATE TEMP TABLE clean_duplicates AS
SELECT * FROM (
    SELECT *,
           -- Without row_id, we just order by the ID to satisfy the syntax and grab the first one
           ROW_NUMBER() OVER(PARTITION BY applicant_id_sk ORDER BY applicant_id_sk) as rn
    FROM aws_project.applicant_optimized
    WHERE
        applicant_id_sk IN (
            SELECT
...
-- 3. Drop the temporary 'rn' column so the schema perfectly matches the main table again
ALTER TABLE clean_duplicates DROP COLUMN rn;

-- 4. Delete ALL versions of these specific students from the main table
DELETE FROM aws_project.applicant_optimized
USING duplicate_ids
WHERE
    aws_project.applicant_optimized.applicant_id_sk = duplicate_ids.applicant_id_sk;

-- 5. Insert the single, clean copies back into the main table
INSERT INTO
aws_project.applicant_optimized
SELECT
    *
...
-- 6. Clean up the temp tables
DROP TABLE duplicate_ids;
DROP TABLE clean_duplicates;
...
SELECT
    COUNT(*)
FROM aws_project.applicant_optimized;-- 1,18,64,129 (This is clean data)

-- Performed some tests with MV created from Zero ETL.
-- Seems these are performing better compared to MV in 8 Qs except the 1st P1.
-- The MV was created as part of Part 3 of this same project.
