...
-- We made changes in Zero ETL pipleline.
-- 1st: MySQL will strictly have proper data that respects Foreign Key, Primay Key which means almost clean data with respect to duplicates.
-- Instead of VIEW + SP combination as shared above, for this we will be using MVs + VIEWs and no SPs.
-- Redshift almost has clean data, as we expect OLTP to provide clean data which is the realtime situation in an enterprise.

SELECT
    integration_id
FROM svv_integration;-- a3ba3d74-2407-488d-8240-f2383ed9554a

-- DROP DATABASE zero_etl_db FORCE;

CREATE DATABASE zero_etl_db
FROM INTEGRATION 'a3ba3d74-2407-488d-8240-f2383ed9554a';-- Took: 1m 45.5s
...
FROM sys_integration_activity
ORDER BY
    last_commit_timestamp DESC LIMIT 10;


DROP MATERIALIZED VIEW public.applicant_institute_gold_mv;
SELECT
    COUNT(*)
FROM public.applicant_institute_gold_mv;


CREATE MATERIALIZED VIEW public.applicant_institute_gold_mv
BACKUP NO
DISTKEY (institute_id_sk)
COMPOUND SORTKEY (applicant_id_sk, institute_id_sk)
AUTO REFRESH YES
AS
SELECT
    -- All Applicant Columns
    ...
    i.declined_no_of_student_pct,
    i.accepted_no_of_student_pct
FROM zero_etl_db.aws_project.applicant AS a
INNER JOIN zero_etl_db.aws_project.institute i
  ON a.institute_id_fk = i.institute_id_sk;--Took: 46.9s



SELECT
    COUNT(*)
FROM public.applicant_institute_gold_mv;-- 1,18,64,129



-- For audit purpose.
CREATE OR REPLACE VIEW public.exception_unassigned_applicants AS
SELECT
    applicant_id_sk,
    applicant_name,
    applicant_gender,
    applicant_dob,
    applicant_country,
    institute_id_fk AS missing_institute_id,
    error_reason,
    logged_at
FROM zero_etl_db.aws_project.applicant_exceptions
WITH NO SCHEMA BINDING;



-- For audit purpose.
CREATE OR REPLACE VIEW public.exception_empty_institutes AS
SELECT
    i.institute_id_sk,
    i.institute_name,
    i.applied_no_of_students,
    i.declined_no_of_student_pct
FROM zero_etl_db.aws_project.institute i
LEFT JOIN zero_etl_db.aws_project.applicant a
  ON i.institute_id_sk = a.institute_id_fk
WHERE
    a.applicant_id_sk IS NULL
WITH NO SCHEMA BINDING;
...
SELECT
    COUNT(*)
FROM public.applicant_institute_gold_mv;-- 1,18,64,129
-- 1,41,99,996 (original) - 7,05,038 (doubles) = 1,34,94,958
-- 1,34,94,958 (unique applicant_id_sk) - 16,30,829 (orphans) = 1,18,64,129 (clean: de-duplicated + orphan less)
...
aws_project   applicant_optimized         Y, AUTO(ENCODE)   KEY(institute_id_fk) institute_id_fk        1.34        0.67          4476      0.0069
public        applicant_institute_gold_mv Y                 KEY(institute_id_sk) applicant_id_sk        1.34        1.43          7791      0.0121
aws_project   institute                   Y, AUTO(ENCODE)   AUTO(KEY(row_id))    AUTO(SORTKEY(row_id))  1.1         1             3840      0.006
aws_project   applicant                   Y, AUTO(ENCODE)   AUTO(KEY(row_id))    AUTO(SORTKEY(row_id))  1.02        1             4608      0.0072


-- We sent some manual updates into applicant and institute tables of MySQL and the changes are reflecting within few seconds.
...
FROM public.applicant_institute_gold_mv
WHERE
    applicant_id_sk = 200012;-- UPDATE

SELECT
    *
FROM public.applicant_institute_gold_mv
WHERE
    applicant_id_sk = 9999997;-- INSERT

SELECT
    *
FROM public.exception_unassigned_applicants
WHERE
    applicant_id_sk = 99999999;-- INVALID DATA



SELECT
    COUNT(*)
FROM public.applicant_institute_gold_mv;-- 1,18,64,130
