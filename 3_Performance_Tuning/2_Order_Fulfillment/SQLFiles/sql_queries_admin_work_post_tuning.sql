...
FROM svv_table_info
WHERE
    table_name IN ('order_fulfillment_stg', 'order_fulfillment_def', 'order_fulfillment_opt', 'order_fulfillment_hybrid')
ORDER BY
    skew_rows DESC;
...
order_fulfillment_hybrid    Y, AUTO(ENCODE)     KEY(fc_id)              fc_release_to_mfg_date  1.16        0.78            17972       0.028
...
order_fulfillment_hybrid    0           0           0
*/
...
FROM pg_table_def
WHERE
    tablename IN ('order_fulfillment_stg', 'order_fulfillment_def', 'order_fulfillment_opt', 'order_fulfillment_hybrid')
ORDER BY
    tablename,
    "column";
...
WHERE "schema" = 'aws_project'
    AND "table" IN ('order_fulfillment_stg', 'order_fulfillment_def', 'order_fulfillment_opt', 'order_fulfillment_hybrid');


/*
table_name                  total_records   total_megabytes     records_per_mb
order_fulfillment_hybrid    250000000       17972               13910.5274
...
WHERE
    ...
    AND tablename IN ('order_fulfillment_stg', 'order_fulfillment_def', 'order_fulfillment_opt', 'order_fulfillment_hybrid')
ORDER BY
    tablename;
