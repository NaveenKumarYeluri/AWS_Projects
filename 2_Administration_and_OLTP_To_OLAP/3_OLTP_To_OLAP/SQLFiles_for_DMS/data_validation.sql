...
-- 1. Part 1 (Physical Optimized Tables)
SELECT
    a.applicant_id_sk, a.applicant_name, a.applicant_gender, a.applicant_country,
    TRUNC(a.applicant_dob) AS date_of_birth,
    a.applicant_high_school_GPA, a.applicant_qual_test_score, a.interview_score,
    a.scholarship_grade, a.scholarship_pct,
    a.course_name, a.admission_date, i.institute_name, i.institute_fee,
...
LEFT JOIN aws_project.institute_optimized AS i
    ON i.institute_id_sk = a.institute_id_fk
WHERE a.applicant_id_sk = 200006;--Took: 9 ms

-- 2. Zero-ETL (Materialized View)
SELECT
    applicant_id_sk, applicant_name, applicant_gender, applicant_country,
    TRUNC(applicant_dob) AS date_of_birth,
    applicant_high_school_pct, applicant_qual_test_score, interview_score,
    scholarship_grade, scholarship_pct,
    course_name, admission_date, institute_name, institute_fee,
    (institute_fee - (institute_fee * (scholarship_pct / 100.0))) AS final_payable_fee
FROM applicant_institute_gold_mv
WHERE applicant_id_sk = 200006;--Took: 68 ms

-- 3. DMS / Lakehouse (External S3 Table)
SELECT
    applicant_id_sk, applicant_name, applicant_gender, applicant_country,
    TRUNC(applicant_dob) AS date_of_birth,
    applicant_high_school_GPA, applicant_qual_test_score, interview_score,
    scholarship_grade, scholarship_pct,
    course_name, admission_date, institute_name, institute_fee,
    (institute_fee - (institute_fee * (scholarship_pct / 100.0))) AS final_payable_fee
FROM lakehouse.dms_applicant_institute_gold
WHERE applicant_id_sk = 200006;--Took: 457 ms
...
JOIN aws_project.institute_optimized AS i
    ON i.institute_id_sk = a.institute_id_fk
WHERE a.interview_score > i.score_cut_off
GROUP BY i.institute_name
ORDER BY students_cnt DESC;--Took: 9 ms
...
FROM applicant_institute_gold_mv
WHERE interview_score > score_cut_off
GROUP BY institute_name
ORDER BY students_cnt DESC;--Took: 32 ms
...
WHERE interview_score > score_cut_off
GROUP BY institute_name
ORDER BY students_cnt DESC;--Took: 1290 ms
...
WHERE institute_id_sk = 'TISN707';--Took: 8 ms
...
SELECT DISTINCT
    ...
    accepted_no_of_student_pct
FROM applicant_institute_gold_mv
WHERE institute_id_sk = 'TISN707';--Took: 68 ms
...
SELECT DISTINCT
    ...
    accepted_no_of_student_pct
FROM lakehouse.dms_applicant_institute_gold
WHERE institute_id_sk = 'TISN707';--Took: 614 ms
...
WHERE applicant_id_sk = 200006;--Took: 9 ms
...
SELECT
    ...
    applicant_qual_test_score AS qualification_score
FROM applicant_institute_gold_mv
WHERE applicant_id_sk = 200006;--Took: 69 ms
...
SELECT
    ...
    applicant_qual_test_score AS qualification_score
FROM lakehouse.dms_applicant_institute_gold
WHERE applicant_id_sk = 200006;--Took: 487 ms
...
FROM aws_project.institute_optimized
ORDER BY declined_no_of_student_pct DESC
LIMIT 5;--Took: 6 ms
