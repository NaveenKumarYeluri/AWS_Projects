...
-- Full data is loaded and the results are here:
...
ORDER BY
    institute_name,
    applicant_name;-- Took: 25 ms (after multiple runs)
...
FROM applicant_institute_gold_mv
WHERE
    applicant_id_sk = 200006;-- Took: 75 ms
...
-- Part 1: Architecturehighly normalized and physically optimized, making it incredible for point-lookups, filtering specific IDs, and keeping storage costs low.
-- Part 3 (Zero ETL): Materialized Views are best reserved for massive, heavy-aggregation BI dashboards (e.g., "Give me the average scholarship percentage grouped by country and college for the last 5 years").
